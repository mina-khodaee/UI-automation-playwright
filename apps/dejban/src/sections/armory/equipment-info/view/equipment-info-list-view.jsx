...
import { EquipmentInfoNewEditForm } from '../equipment-info-new-edit-form';

export function EquipmentInfoListView() {

  const { t: t_majors } = useTranslate('major');
  const { t: t_common, currentLang } = useTranslate();

  const [openDialog, setOpenDialog] = useState(false);
  const [currentEquipment, setCurrentMajor] = useState(null);
  ...
  // const allEquipment = data?.items || [];

  const allEquipment = [
    { name: 'تجهیزات 1', code: 1, description: 'تست' }
  ]
