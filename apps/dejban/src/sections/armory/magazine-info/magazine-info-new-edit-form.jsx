...
export function MagazineInfoNewEditForm({ currentMagazine, open, onClose, onRefetch }) {
    ...
    const defaultValues = useMemo(
        () => ({
            ...
            name: currentMagazine?.name || '',
            code: currentMagazine?.code || '',
        }),
        [currentMagazine]
    );
    ...
    const onSubmit = handleSubmit(async (data) => {
        try {
            if (currentMagazine) {
                await updateMajors.mutateAsync({ id: currentMagazine.id, ...data });
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            <DialogTitle textAlign="center">
                {currentMagazine ? 'ویرایش خشاب' : 'ایجاد خشاب'}
            </DialogTitle>
                ...
                <Form methods={methods} onSubmit={onSubmit}>
                    ...
                    <Stack justifyContent="flex-end" direction='row' gap={1} sx={{ mt: 3 }}>
                        <Button
                            ...
                            size="small"
                        >
                            {currentMagazine ? t_common('button.update') : t_common('button.create')}
                        </Button>
