...
import { AmmunitionInfoNewEditForm } from '../ammunition-info-new-edit-form';

export function AmmunitionInfoListView() {

  const { t: t_majors } = useTranslate('major');
  const { t: t_common, currentLang } = useTranslate();

  const [openDialog, setOpenDialog] = useState(false);
  const [currentAmmunition, setCurrentMajor] = useState(null);
  ...
  // const allAmmunition = data?.items || [];

  const allAmmunition = [
    { name: 'مهمات 1', code: 1, description: 'تست' }
  ]
