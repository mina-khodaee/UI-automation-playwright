...
export function WeaponInfoNewEditForm({ currentWeapon, open, onClose, onRefetch }) {
    ...
    const onSubmit = handleSubmit(async (data) => {
        try {
            if (currentWeapon) {
                await updateMajors.mutateAsync({ id: currentWeapon.id, ...data });
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            <DialogTitle textAlign="center">
                {currentWeapon ? 'ویرایش اسلحه' : 'ایجاد اسلحه'}
            </DialogTitle>
                ...
                <Form methods={methods} onSubmit={onSubmit}>
                    ...
                    <Stack justifyContent="flex-end" direction='row' gap={1} sx={{ mt: 3 }}>
                        <Button
                            ...
                            size="small"
                        >
                            {currentWeapon ? t_common('button.update') : t_common('button.create')}
                        </Button>
