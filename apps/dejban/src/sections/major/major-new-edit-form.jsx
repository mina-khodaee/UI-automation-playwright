...
export function MajorNewEditForm({ currentMajor, open, onClose, onRefetch }) {
    ...
    const onSubmit = handleSubmit(async (data) => {
        try {
            if (currentMajor) {
                await updateMajors.mutateAsync({ id: currentMajor.id, ...data });
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            <DialogTitle textAlign="center">
                {currentMajor ? t_majors('title.updateMajors') : t_majors('title.insertMajors')}
            </DialogTitle>
                ...
                <Form methods={methods} onSubmit={onSubmit}>
                    <Grid container spacing={3} justifyContent="center" alignItems="center">
                        <Grid size={12}>
                            ...
                            >
                                <Field.Text
                                    name="name"
                                    label={t_majors('formsInputs.name')}
                                ...
                                <Field.Text
                                    name="code"
                                    label={t_majors('formsInputs.code')}
                                    inputProps={{ type: 'text' }}
                                    size="small"
                                />
                            </Box>
                            ...
                            >
                                ...
                                <Field.Text
                                    name="description"
                                    label={t_majors('formsInputs.description')}
                                    multiline
                                    rows={3}
                                    size="small"
                                />
                    ...
                    <Stack justifyContent="flex-end" direction='row' gap={1} sx={{ mt: 3 }}>
                        <Button
                            ...
                            size="small"
                        >
                            {currentMajor ? t_common('button.update') : t_common('button.create')}
                        </Button>
