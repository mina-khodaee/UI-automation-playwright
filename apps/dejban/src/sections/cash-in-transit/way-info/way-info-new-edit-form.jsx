...
export function WayInfoNewEditForm({ currentWay, open, onClose, onRefetch }) {
    ...
    const defaultValues = useMemo(
        () => ({
            description: currentWay?.description || '',
    ...
    const onSubmit = handleSubmit(async (data) => {
        try {
            if (currentWay) {
                await updateMajors.mutateAsync({ id: currentWay.id, ...data });
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            <DialogTitle textAlign="center">
                {currentWay ? 'ویرایش مسیر' : 'ایجاد مسیر'}
            </DialogTitle>
                ...
                <Form methods={methods} onSubmit={onSubmit}>
                    ...
                    <Stack justifyContent="flex-end" direction='row' gap={1} sx={{ mt: 3 }}>
                        <Button
                            ...
                            size="small"
                        >
                            {currentWay ? t_common('button.update') : t_common('button.create')}
                        </Button>
