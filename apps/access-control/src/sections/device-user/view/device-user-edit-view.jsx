...
export function DeviceUSerEditView({ deviceUser, deviceUserLoading }) {
  const {t} = useTranslate('user');
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading={t('breadcrumb.updateDeviceUser')}
        links={[
          ...
          { name: t('breadcrumb.updateDeviceUser') },
        ]}
        sx={{ mb: { xs: 3, md: 5 } }}
      />
        { !deviceUserLoading && (
          <DeviceUserNewEditForm currentDeviceUser={deviceUser} />
        )
      }
    </DashboardContent>
  );
